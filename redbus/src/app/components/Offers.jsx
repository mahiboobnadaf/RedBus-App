function Offers() {
  const offers = [
      ...
      { id: 4, title: 'Special Weekend Offer', description: 'Up to ₹200 off on weekend bookings!', banner: '/static/fourthOffer.png' },
  ];

  return (
      <div className="p-6 bg-gray-100">
          <h2 className="text-2xl font-bold mb-6 text-gray-800">Offers</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
              {offers.map((offer) => (
                  <div
                      key={offer.id}
                      className="relative bg-white border border-gray-200 rounded-lg shadow-md overflow-hidden hover:shadow-xl transition-shadow duration-300"
                  >
                      <img
                          src={offer.banner}
                          alt={offer.title}
